...
def get_com_vector(robot):
    theta = abs(
        radians(robot.get_pitch()) - start_angle
    )  # Current robot angle relative to starting surface
    alpha = atan(
        2 * robot.get_h() / robot.get_l()
    )  # Angle between CoM vector and robot pitch
    angle_vector = np.matrix(
        [cos(alpha + theta)], [sin(alpha + theta)]
    )  # Direction of vector from rear wheel center to CoM
    wheel_offset = np.matrix([0], [robot.get_r()])  # Offset for wheel radius
    r_com = (
        sqrt(robot.get_h() ** 2 + (robot.get_l() / 2) ** 2) * angle_vector
        + wheel_offset
    )
    return r_com
...
def get_gravity_rot_mat():
    R = np.matrix(
        [cos(start_angle), -sin(start_angle)], [sin(start_angle), cos(start_angle)]
    )
    return R
...
def get_contact_rot_mat():
    R = np.matrix(
        [cos(transition_angle), -sin(transition_angle)],
        [sin(transition_angle), cos(transition_angle)],
    )
    return R
...
def solve_climb(robot):
    ...
    A_eq = [
        [
            0,
            sin(transition_angle),
            -sin(transition_angle),
            0,
            0,
            cos(transition_angle),
            1,
        ],
        [
            0,
    ...
    b_eq = [
        M * (1 + cos(transition_angle)) + m * g_rot[1],
    ...
    if res.success:
        return [
            robot.get_r() * (res.x[1] - res.x[2]),
            robot.get_r() * (res.x[3] - res.x[4]),
        ]  # Desired wheel torques
    else:
        return None
...
def loop(robot):
    if state == "TELEOP":
        ...
        if transition_angle - abs(radians(robot.get_pitch()) - start_angle) > radians(
            2
        ):
