...
class Motors:
    ...
    def add(
        ...
        if not full_model:
            if series in SERIES:
                ...
                print(
                    "Unsupported motor series " + series + ", motors could not be added"
                )
                return
        if not offset:
            offset = {}
    ...
    def enable(self, ids=None, velocity_mode=False, torque_mode=False):
        ...
        for series, motor_list in self.motors_by_series.items():
            motor_list = list(
                filter(lambda m: m.id in ids or ids[0] is None, motor_list)
            )
            if series == "AX":
    ...
    def write_angle(self, ids=None):
        ...
        for series, motor_list in self.motors_by_series.items():
            ...
            if series == "AX":
                ...
                for motor in motor_list:
                    raw = (
                        ((motor.set_angle + motor.offset) * motor.mirror + 150)
                        * 1023.0
                        / 300.0
                    )
                ...
                for motor in motor_list:
                    raw = (
                        ((motor.set_angle + motor.offset) * motor.mirror + 180)
                        * 4095.0
                        / 360.0
                    )
                    sync.addParam(motor.id, float2bytes(raw, sync.data_length))
            ...
            if series == "AX":
                ...
                for motor in motor_list:
                    ...
                    if motor.torque_mode:
                        raw = (
                            min(motor.set_torque * motor.mirror, motor.stall)
                            * 2300
                            / motor.stall
                            / 2.69
                        )
                        ...
                        raw = (
                            min(abs(motor.set_torque), motor.stall)
                            * 2300
                            / motor.stall
                            / 2.69
                        )
...
class Motor:
    def __init__(
        ...
        self.torque_list = (
            0.0,
        ) * average_torque  # List of the last average_torque torques
...
if __name__ == "__main__":
    ...
    motors.add(
        [1, 2, 3, 4], "XM430-W350-T", lower=-50, upper=50
    )  # Add 4 XM series motors
    motors.add(
        [5, 6, 7, 8], "AX18-A", lower=-70, upper=70, mirror=(7, 8)
    )  # Add 4 AX series motors
